...
std::vector<ConstitutionAttributes> loadConstitutionAttributes(const std::string& filename)
{
	std::ifstream i("constitution.json");

	if (!i.is_open()) {
		std::cerr << "Error: Unable to open constitution.json. Error " << std::to_string(errno) << std::endl;
