...
bool StatBoostEquipment::use(Item& item, Creature& wearer, GameContext& ctx)
{
	...
	if (success)
	{
		if (wasEquipped)
		{
			...
			{
				if (str_bonus != 0)
					wearer.set_strength(original_stats.str);
				if (dex_bonus != 0)
					wearer.set_dexterity(original_stats.dex);
				if (con_bonus != 0)
					wearer.set_constitution(original_stats.con);
				if (int_bonus != 0)
					wearer.set_intelligence(original_stats.intel);
				if (wis_bonus != 0)
					wearer.set_wisdom(original_stats.wis);
				if (cha_bonus != 0)
					wearer.set_charisma(original_stats.cha);
			}
			else
			{
		...
		else
		{
			...
			if (is_set_mode)
			{
				if (str_bonus != 0)
				{
					original_stats.str = wearer.get_strength();
					wearer.set_strength(str_bonus);
				}
				if (dex_bonus != 0)
				{
					original_stats.dex = wearer.get_dexterity();
					wearer.set_dexterity(dex_bonus);
				}
				if (con_bonus != 0)
				{
					original_stats.con = wearer.get_constitution();
					wearer.set_constitution(con_bonus);
				}
				if (int_bonus != 0)
				{
					original_stats.intel = wearer.get_intelligence();
					wearer.set_intelligence(int_bonus);
				}
				if (wis_bonus != 0)
				{
					original_stats.wis = wearer.get_wisdom();
					wearer.set_wisdom(wis_bonus);
				}
				if (cha_bonus != 0)
				{
					original_stats.cha = wearer.get_charisma();
					wearer.set_charisma(cha_bonus);
				}
			}
			else
			{
...
void StatBoostEquipment::apply_stat_bonuses(Creature& creature, Item& owner, GameContext& ctx)
{
	if (owner.has_state(ActorState::IS_EQUIPPED))
	{
		...
		{
			if (str_bonus != 0)
				creature.set_strength(original_stats.str);
			if (dex_bonus != 0)
				creature.set_dexterity(original_stats.dex);
			if (con_bonus != 0)
				creature.set_constitution(original_stats.con);
			if (int_bonus != 0)
				creature.set_intelligence(original_stats.intel);
			if (wis_bonus != 0)
				creature.set_wisdom(original_stats.wis);
			if (cha_bonus != 0)
				creature.set_charisma(original_stats.cha);
		}
		else
		{
	...
	else
	{
		...
		if (is_set_mode)
		{
			if (str_bonus != 0)
			{
				original_stats.str = creature.get_strength();
				creature.set_strength(str_bonus);
			}
			if (dex_bonus != 0)
			{
				original_stats.dex = creature.get_dexterity();
				creature.set_dexterity(dex_bonus);
			}
			if (con_bonus != 0)
			{
				original_stats.con = creature.get_constitution();
				creature.set_constitution(con_bonus);
			}
			if (int_bonus != 0)
			{
				original_stats.intel = creature.get_intelligence();
				creature.set_intelligence(int_bonus);
			}
			if (wis_bonus != 0)
			{
				original_stats.wis = creature.get_wisdom();
				creature.set_wisdom(wis_bonus);
			}
			if (cha_bonus != 0)
			{
				original_stats.cha = creature.get_charisma();
				creature.set_charisma(cha_bonus);
			}
		}
		else
		{
