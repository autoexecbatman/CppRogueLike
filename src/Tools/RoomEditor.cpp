...
void RoomEditor::handle_input_normal(GameContext& ctx)
{
    ...
    // Global zoom
    if (IsKeyPressed(KEY_EQUAL))  { r.zoom_in();  return; }
    if (IsKeyPressed(KEY_MINUS))  { r.zoom_out(); return; }
    ...
    if (ctrl && IsKeyPressed(KEY_UP))    { depth_min = std::max(1, depth_min - 1); return; }
    if (ctrl && IsKeyPressed(KEY_DOWN))  { depth_min = std::min(depth_min + 1, depth_max); return; }
    if (ctrl && IsKeyPressed(KEY_RIGHT)) { depth_max = std::min(99, depth_max + 1); return; }
    if (ctrl && IsKeyPressed(KEY_LEFT))  { depth_max = std::max(depth_min, depth_max - 1); return; }
...
) const
{
    ...
    if (mouse_px < area_x || mouse_px >= area_x + area_w) return false;
    if (mouse_py < area_y || mouse_py >= area_y + area_h) return false;
    ...
    if (out_cx < 0 || out_cx >= canvas_w) return false;
    if (out_cy < 0 || out_cy >= canvas_h) return false;
...
int RoomEditor::screen_to_list_index(const Renderer& r, int mouse_px, int mouse_py) const
{
    ...
    if (mouse_px < panel_x || mouse_px >= r.get_screen_width()) return -1;
    if (mouse_py < panel_y || mouse_py >= panel_y + panel_h)    return -1;
...
int RoomEditor::screen_to_palette_index(const Renderer& r, int mouse_px, int mouse_py) const
{
    ...
    if (mouse_px < 0 || mouse_px >= panel_w)             return -1;
    if (mouse_py < panel_y || mouse_py >= panel_y + panel_h) return -1;
...
int RoomEditor::symbol_tile_id(char sym) const
{
    switch (sym)
    {
    case '#': return TILE_WALL_STONE;
    case '.': return TILE_FLOOR_STONE;
    case ',': return TILE_CORRIDOR;
    case '+': return TILE_DOOR_CLOSED;
...
void RoomEditor::render_left_panel(const Renderer& r) const
{
    ...
    for (int slot = 0; slot < max_visible && (scroll_start + slot) < pal_size; ++slot)
    {
        ...
        if (is_structural)
        {
            ...
            {
                Color block{};
                if (sym == '#')      block = Color{ 90,  90, 100, 255 };
                else if (sym == '.') block = Color{ 70,  55,  40, 255 };
                else if (sym == ',') block = Color{ 55,  55,  60, 255 };
                else if (sym == '+') block = Color{100,  70,  40, 255 };
                else if (sym == '~') block = Color{ 30,  60, 120, 255 };
                else                 block = Color{ 50,  50,  50, 255 };
...
void RoomEditor::render_right_panel(const Renderer& r) const
{
    ...
    {
        r.draw_text_color(
            panel_x + 6, panel_y + list_y_off + 4,
            "No prefabs yet.",
...
void RoomEditor::handle_input_picker(GameContext& ctx)
{
    ...
    if (IsKeyPressed(KEY_LEFT))  picker_col = (picker_col - 1 + cols) % cols;
    if (IsKeyPressed(KEY_RIGHT)) picker_col = (picker_col + 1)        % cols;
    if (IsKeyPressed(KEY_UP))    picker_row = (picker_row - 1 + rows) % rows;
    if (IsKeyPressed(KEY_DOWN))  picker_row = (picker_row + 1)        % rows;
    ...
    if (IsKeyPressed(KEY_ENTER) && library)
    {
        ...
        if (palette_index >= 0 && palette_index < static_cast<int>(pal.size()))
        {
            ...
            library->save(Paths::PREFABS);
            set_status(std::format("'{}' -> tile ({},{}) sheet {}", sym,
                                   picker_col, picker_row,
...
void RoomEditor::render_tile_picker(const Renderer& r) const
{
    ...
    std::string header = std::format(
        ...
        sym_info,
        picker_sheet_list_idx + 1, PICKER_SHEET_COUNT,
        picker_col, picker_row,
    ...
    std::string sheet_label = std::format("Sheet: {}  ({} cols x {} rows)",
        PICKER_SHEET_NAMES[picker_sheet_list_idx], cols, rows);
