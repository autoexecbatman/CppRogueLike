...
static Vector2D v(int col, int row) noexcept { return Vector2D{ col, row }; }
...
) const
{
    ...
    for (int gr = 0; gr < grid_rows; ++gr)
    {
        for (int gc = 0; gc < grid_cols; ++gc)
        {
            ...
            if (room_col < 1 || room_row < 1)                 continue;
            if (room_col + room_w > map_width  - 2)            continue;
            if (room_row + room_h > map_height - 2)            continue;
