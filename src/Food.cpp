...
bool Food::use(Item& owner, Creature& wearer) {
    ...
    game.appendMessagePart(GOLD_PAIR, owner.actorData.name);
...
Ration::Ration(Vector2D position) : Item(position, ActorData{ '%', "ration", HPBARFULL_PAIR }) {
    pickable = std::make_unique<Food>(300);  // High nutritional value
    value = 10;  // Base gold value
}

Fruit::Fruit(Vector2D position) : Item(position, ActorData{ '%', "fruit", TROLL_PAIR }) {
    pickable = std::make_unique<Food>(100);  // Medium nutritional value
    value = 3;  // Base gold value
}

Bread::Bread(Vector2D position) : Item(position, ActorData{ '%', "bread", DRAGON_PAIR }) {
    pickable = std::make_unique<Food>(200);  // Medium-high nutritional value
    value = 5;  // Base gold value
}

Meat::Meat(Vector2D position) : Item(position, ActorData{ '%', "meat", ORC_PAIR }) {
