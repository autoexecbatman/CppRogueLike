...
private:
	...
	static constexpr int calculate_xp_for_level(int level, const std::array<int, N>& xpTable, int linearProgression) noexcept
	{
...
public:
	...
	// Virtual confusion hook -- no-op for AI types that manage confusion internally.
	// AiPlayer overrides to set confusionTurns. Default: wraps via Creature::apply_confusion.
	virtual void apply_confusion(int /*duration*/) {}
...
protected:
	...
	enum class AiType
	{
		...
		PLAYER,
		SHOPKEEPER,
		MIMIC // append last — integer values are serialized
	};
};
